...
class Encoder(nn.Module):
    def __init__(
        ...
        enc_norm = nn.LayerNorm(emb_dim)
        
        self.transformer_enc = nn.TransformerEncoder(transformer_enc_layer, num_enc_layers, enc_norm)
...
class Decoder(nn.Module):
    def __init__(
        ...
        dec_norm = nn.LayerNorm(emb_size)
    ...
    def forward(self, src, memory, trg):
        ...
        trg_embedded = self.dropout(self.trg_pos_dec(trg_pos) + trg_embedded)
        # trg_embedded = [trg sent len, batch size, d_model]
